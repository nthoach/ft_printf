...
/*   Updated: 2023/11/25 21:46:45 by nthoach          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	// int		n_p = 0;
	// n_p = printf("Printing a string
	//:%c:%s:%s:%p:%p:%d:%i:%u:%x:%X:%%:
	//\n",'\t', s, NULL, s, s1, -2147483647,
	// INT_MAX, UINT_MAX, 14, UINT_MAX);
	// printf(":printed length = %i:\n", n_p);
	// n_p = ft_printf("Printing a string:%c
	//:%s:%s:%p:%p:%d:%i:%u:%x:%X:%%:\n",'\t', s,
	// NULL, s, s1, -2147483647, INT_MAX, UINT_MAX, 14, UINT_MAX);
